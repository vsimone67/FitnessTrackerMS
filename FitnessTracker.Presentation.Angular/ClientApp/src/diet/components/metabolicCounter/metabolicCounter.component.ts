...
@Component({
    ...
    templateUrl: 'metabolicCounter.component.html'
})
export class MetabolicCounter implements OnInit {
    ...
    ngOnChanges(changes: SimpleChange) {
        ...
        if (this.macroColumns.length > 0) {
            ...
            this.macroColumns.forEach((column) => {
                this.meals.push(new NutritionInfo(column.MealId, column.MealDisplayName));
            });
    ...
    calcTotals() {
        this._dietervice.getcurrentMacro(this.mode.value, (metabolicInfo: CurrentMacros) => {
