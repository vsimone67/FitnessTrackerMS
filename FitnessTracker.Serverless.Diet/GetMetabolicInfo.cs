...
{
    public static class GetMetabolicInfo
    {
        [FunctionName("GetMetabolicInfo")]
        public static async Task<IActionResult> Run([HttpTrigger(AuthorizationLevel.Anonymous, "get", Route = null)] HttpRequest req, ILogger log, ExecutionContext context)
        {
            ...
            try
            {
                ...
                var queryHanlder = new GetMetabolicInfoQueryHandler(ftEnvironment.Service, ftEnvironment.Mapper);
                var results = queryHanlder.Handle(new GetMetabolicInfoQuery(), new System.Threading.CancellationToken());
